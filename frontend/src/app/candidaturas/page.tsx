  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between flex-wrap gap-2">
        <h1 className="text-2xl font-bold text-stone-100 tracking-tight">Candidaturas</h1>
        <div className="flex gap-4 text-sm text-stone-400">
          <span>{total} total</span>
          <span>{respostas} respostas</span>
          <span className="font-semibold text-amber-300">{taxa}% callback</span>
        </div>
      </div>

      {apps.length === 0 ? (
        <div className="card border-dashed text-center py-16 text-stone-500">
            ...
            <div key={app.id} className="card p-4 flex items-center gap-4">
              <div className="flex-1 min-w-0">
                <a
                  ...
                  className="font-semibold text-stone-100 truncate hover:text-amber-300"
                >
                  {app.jobTitle}
                </a>
              ...
              <div className="shrink-0 flex items-center gap-2">
                ...
                <select
                  ...
                  className="text-xs bg-[#16130e] border border-[#2b251c] text-stone-300 rounded-lg px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-amber-500/50"
                >
                  {STAGES.map((s) => (
